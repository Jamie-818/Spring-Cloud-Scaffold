...
public class RestTemplateController {
  ...
  /**
   * 第一种方式(直接使用 RestTemplate,URL 写死) @Method: GET
   *
   * <p>缺点：无法负载均衡</>
   *
   * @author show
  ...
  public String getRibbonServer1() {
    ...
    String response =
        restTemplate.getForObject(
            "http://localhost:8000/RibbonServer/RibbonTest?requestMsg=" + requestMsg, String.class);
    log.info("response={}", response);
    return response;
  }

  /**
   * 第一种方式(直接使用 RestTemplate,URL 写死) @Method: POST
   *
   * <p>缺点：无法负载均衡</>
   *
   * @author show
  ...
  public String postRibbonServer1() {
    ...
    String response =
        restTemplate.postForObject(
            "http://localhost:8000/RibbonServer/RibbonTest",
            getRequestParam(requestMsg),
            String.class);
    log.info("response={}", response);
    return response;
  }
  ...
  /**
   * 第二种方式(利用 LoadBalancerClient 通过应用名获取 url，然后再使用 RestTemplate 请求) @Method: GET
   *
   * @author show
   * @date 15:17 2019/6/9
   */
  ...
  public String getRibbonServer2() {
    ...
    String url =
        String.format(
            "http://%s:%s",
            choose.getHost(),
            choose.getPort() + "/RibbonServer/RibbonTest?requestMsg=" + requestMsg);
    RestTemplate restTemplate = new RestTemplate();
  ...
  /**
   * 第二种方式(利用 LoadBalancerClient 通过应用名获取 url，然后再使用 RestTemplate 请求) @Method: POST
   *
   * @author show
   * @date 15:17 2019/6/9
   */
  ...
  public String postRibbonServer2() {
    ...
    String url =
        String.format(
            "http://%s:%s", choose.getHost(), choose.getPort() + "/RibbonServer/RibbonTest");
    RestTemplate restTemplate = new RestTemplate();
  ...
  /**
   * 第三种方式(利用 @LoadBalanced，可在 RestTemplate 里面直接使用应用名字) 1.新建 RestTemplateConfig 类 2.添加 RestTemplate
   * Bean 3.添加 @LoadBalanced注解， 4.注入 RestTemplate 对象 @Method: GET
   *
   * @author show
   * @date 15:17 2019/6/9
   */
  ...
  public String getRibbonServer3() {
    ...
    String response =
        restTemplate.getForObject(
            "http://SHOW-SERVER-EXAMPLE/RibbonServer/RibbonTest?requestMsg=" + requestMsg,
            String.class);
    log.info("response={}", response);
    return response;
  }
  ...
  public String postRibbonServer3() {
    ...
    String response =
        restTemplate.postForObject(
            "http://SHOW-SERVER-EXAMPLE/RibbonServer/RibbonTest",
            getRequestParam(requestMsg),
            String.class);
    log.info("response={}", response);
    return response;
